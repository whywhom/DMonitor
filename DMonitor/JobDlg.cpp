...
BEGIN_MESSAGE_MAP(CJobDlg, CDialog)
	...
	ON_COMMAND(ID_REMOVE_TOOL, &CJobDlg::OnRemoveTool)
	...
	ON_NOTIFY(NM_RCLICK, IDC_ZCW_JOBTREE_LEFT, &CJobDlg::OnNMRClickZcwJobtreeLeft)
	ON_WM_CLOSE()
END_MESSAGE_MAP()
...
void CJobDlg::JobAdd(CString jobName)
{
	try{				
		...
		if(jobName!=""){
			CDaoRecordset rs(&m_DataBase);				
			rs.Open(dbOpenDynaset,_T("SELECT * FROM JobList where JobName='"+jobName+"'"));	
			if(rs.IsBOF()){
				rs.AddNew();
				...
				MessageBox(_T("作业")+jobName+_T("已经存在"));
			}
		}else{
			MessageBox(_T("作业名不能为空"));
		}
	} catch(CDaoException *e){
		MessageBox(e->m_pErrorInfo->m_strDescription);
		e->Delete();
...
void CJobDlg::OnJobLoad()
{
	...
	if(!hPar&&sel_job)
	{
		...
		{
			theApp.LoadJobName = JobName;
			LoadJob(JobName);	
			bNormalClose = false;//非用户关闭窗体，由加载作业操作完成后自动关闭
			//通知接收者处理
			::PostMessage(fatherHwnd,WM_JOBLOAD_OK, 0x00, 0x0000);
...
void CJobDlg::LoadJob(CString JobName){
	...
	while(!rsJob.IsEOF()){					
		...
		while(!rs.IsEOF()){
			...
			_bstr_t bstr_c_tool(ToolLabel);  //增加所属Tool标记
			std::string str_c_tool(bstr_c_tool);  
			itemCurve["Tool"]=str_c_tool;
...
void CJobDlg::OnRemoveTool()
{
	OnBnClickedZcwDelitem();
}
...
void CJobDlg::OnToolCopy()
{
	...
	if(!m_treeCtrl.FindSubDir(strPath)){//不能复制目录
		...
		if(m_jcDlg.DoModal()==IDOK){
			CString strNewPath=m_treeCtrl.GetFullPath(par_htem)+_T("\\")+m_jcDlg.m_newName+_T(".tools");//新仪器路径
			CDaoRecordset rs(&m_DataBase);						
			rs.Open(dbOpenDynaset,_T("SELECT * FROM ToolList where LABEL='")+m_jcDlg.m_newName+_T("'"));//新仪器名在数据库中是否存在
			if(GetFileAttributes(strNewPath)==-1L&&rs.IsBOF()){
				CopyFile(strPath,strNewPath,TRUE);//复制文件
				//仪器名称入库	
				rs.AddNew();
				rs.SetFieldValue(0,COleVariant(m_jcDlg.m_newName));
				rs.Update();
				rs.Close();	
				m_treeCtrl.AddItem(par_htem,strNewPath);//添加节点
			}else{
				MessageBox(m_jcDlg.m_newName+_T("已存在"),MB_OK);
			}
		}
	}
}
...
void CJobDlg::OnNMRClickZcwJobTree(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	else
	{
		...
		*/
		{
			CString strPath=m_treeCtrl.GetFullPath(titem);//获取文件路径名称
			if(!m_treeCtrl.FindSubDir(strPath))
			{
				CPoint point;
				GetCursorPos(&point);
...
void CJobDlg::OnNMRClickZcwJobtreeLeft(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	CPoint  cp; 
	GetCursorPos(&cp); 
	m_JobTreeLeft.ScreenToClient(&cp);  //m_TREE是CTreeCtrl控件变量。
	HTREEITEM  titem = m_JobTreeLeft.HitTest(cp,NULL); //获取当前鼠标右键单击的位置下的item
	if(titem == 0)
	{
		CPoint point;
		GetCursorPos(&point);
		CMenu menu;
		menu.LoadMenu(IDR_JOB_NEW);
		menu.GetSubMenu(0)->TrackPopupMenu(TPM_LEFTALIGN | 
			TPM_RIGHTBUTTON, point.x, point.y, this, NULL);
	}
	else
	{
		m_JobTreeLeft.SelectItem(titem);
		CString str = m_JobTreeLeft.GetItemText(titem);
		/*
		HTREEITEM sel_job=m_JobTreeLeft.GetSelectedItem(); 
		if(sel_job != titem)
		{
		m_JobTreeLeft.SetItemState(titem,LVIS_SELECTED,LVIS_SELECTED);
		CPoint point;
		GetCursorPos(&point);
		CMenu menu;
		menu.LoadMenu(IDR_JOB_NEW);
		...
		*/
		{
			CString JobName=m_JobTreeLeft.GetItemText(titem);//获取文件路径名称
			HTREEITEM hPar=m_JobTreeLeft.GetParentItem(titem);//父节点
			CPoint point;
			GetCursorPos(&point);
			if(!hPar)
			{
...
void CJobDlg::OnClose()
{
	...
	if(bNormalClose == true)//非用户关闭窗体，由加载作业操作完成后自动关闭
	{
		//通知接收者处理
		CString JobName=theApp.LoadJobName;//获取作业名称
		//必须是一级节点（作业）
		LoadJob(JobName);	
		//通知接收者处理
		::PostMessage(fatherHwnd,WM_JOBLOAD_OK, 0x01, 0x0000);
	}
	CDialog::OnClose();
}
