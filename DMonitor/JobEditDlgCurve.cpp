...
BOOL CJobEditDlgCurve::OnInitDialog()
{
	...
	// TODO:  在此添加额外的初始化
	((CEdit*)GetDlgItem(IDC_ZCW_CURVE_LABEL))->SetLimitText(10);
	((CEdit*)GetDlgItem(IDC_ZCW_CURVE_TITLE))->SetLimitText(10);
	((CEdit*)GetDlgItem(IDC_EDIT_MIN))->SetLimitText(10);
	((CEdit*)GetDlgItem(IDC_EDIT_MAX))->SetLimitText(10);
...
void CJobEditDlgCurve::OnEnChangeEditMin()
{
	...
	nTestEditMin.GetWindowText(strContent);
	double tempMin = _wtof(strContent);
	if(tempMin <= -65535.00)
	{
		MessageBox(_T("最小值超出范围，请重新输入"),MB_OK);
		return;
	}
	if(strContent.IsEmpty() || strContent == _T("-"))
	{
		return;
	}
	UpdateData();
...
void CJobEditDlgCurve::OnEnChangeEditMax()
{
	...
	// TODO:  在此添加控件通知处理程序代码
	CString strContent = _T("");
	nTestEditMax.GetWindowText(strContent);
	double tempMax = _wtof(strContent);
	if(tempMax >= 65536.00)
	{
		MessageBox(_T("最大值超出范围，请重新输入"),MB_OK);
		return;
	}
	if(strContent.IsEmpty() || strContent == _T("-"))
	{
